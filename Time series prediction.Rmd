...
packages <- c("forecast", "KFAS", "xts", "fastDummies", "tsfknn", "MASS", "tidyr", "ggplot2") # librerie
...
```{r}
df <- gather(results, Model, Values, ARIMA:ML)

p <- ggplot(data = df, aes(x=date, y=Values)) +
  geom_line(aes(color = Model), size = 0.5) +
  theme_minimal() +
  labs(x = NULL, y = "Power", title = "December forecasts comparison")
p
